...
$("#formContact").validate({
		...
		messages: {
			nombre: {
				...
				minlength: "*** Ese nombre es demasiado corto"
			},
			email: {
				required: "*** Debe escribir su dirección de email",
				email: "*** Por favor escriba una direcci&oacute;n de email v\u00E1lida"
			},
			telefono: {
				required: "*** Debe escribir un número de teléfono",
				number: "*** No parece ser un número"
			},
			mensaje:{
				required: "*** D&iacute;ganos que quiere",
				minlength: "*** Su comentario es demasiado corto"
			}
		},
		...
		$.ajax({
			...
			success: function(html){
				$('#formContacto').slideUp('slow').delay(4500).slideDown('slow');
				$("#formContacto").after('<div id="display" class="col-md-10">'+html+'</div>');
				$('#display').slideDown('slow').delay(5000).slideUp('slow');
				$('#formContact').each (function(){
					this.reset();
				});
				/*var url = "http://localhost/buscarpiso_codeigniter/login/logout";
